/* global L, GeoSearch, Search, Data, Utils */
...
const AutoComplete = {
  ...
  _placesApi(xhttp) {
    const eventsData = JSON.parse(xhttp.response)
    ...
    for (var i = 0; i < eventsData.length; i++) {
      ...
      if (!venuesAlreadyMarked.includes(venueId)) {
        const marker = L.marker([eventsData[i].latitude, eventsData[i].longitude], { venueId: venueId })
        AutoComplete.currentMarkers.push(marker)
    ...
    AutoComplete.currentMarkersGroup.on('click', function(event) {
      const venueId = event.layer.options.venueId
  ...
  _autocomplete(searchTextInput) {
    ...
    searchTextInput.addEventListener('input', function(_event) {
      if (this.value.length > 1) {
        ...
        AutoComplete.geoSearchService.search(geoSearchRequest).then(function(results, _status) {
          ...
          for (i = 0; i < AutoComplete.searchResults.length; i++) {
            ...
            eachAutoCompleteResultElement.addEventListener('click', function(_event) {
    ...
    AutoComplete.searchCloseButton.addEventListener('click', function(_event) {
