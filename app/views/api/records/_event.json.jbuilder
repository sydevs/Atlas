...
if event.venue.present?
  json.address [event.room, event.venue.name, event.venue.address].compact.join(', ')
  ...
  if event.venue.latitude? && event.venue.latitude?
    ...
    if event.venue.place_id?
      json.directions_url "https://www.google.com/maps/search/?api=1&query=#{event.venue.address}>&query_place_id=#{event.venue.place_id}"
    else
